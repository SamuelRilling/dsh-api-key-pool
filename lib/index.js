...
// `settings` lets this plugin register its settings namespace on the Host. The
// Settings → Plugins tab renders cards for namespaces the Host serves, so this
// registration is what makes the browser-side card (lib/client.js, keyed by
// the same namespace) actually appear.
const inject = ['llm', 'webServer', 'settings']
const API_BASE = '/dsh-api-key-pool'
// Settings namespace this plugin owns. It must match the `key` used by the
// browser card in lib/client.js. The namespace pattern is lowercase kebab-case.
const API_KEY_POOL_NS = 'api-key-pool'
...
function apply(ctx, config) {
  ...
  // 注册 settings 命名空间，让浏览器侧卡片（lib/client.js，key 同为
  // 'api-key-pool'）能出现在 设置 → 插件 标签页。此处用空 schema + 空 base 仅作
  // "已提供服务" 的标记 —— 本插件的真实配置通过自己的 REST API 管理，并不读写
  // settings.yaml。`ctx.inject` 保证 settings 服务就绪后才注册。
  ctx.inject(['settings'], (sctx) => {
    sctx.settings.register(API_KEY_POOL_NS, z.object({}), { base: {} })
  })
